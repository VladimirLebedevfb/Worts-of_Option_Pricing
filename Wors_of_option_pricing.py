...
def get_risk_free_rate(date1):
    ...
    """
    try:
        irx = yf.download('^IRX', start=date1, end=pd.to_datetime(date1) + pd.Timedelta(days=1))['Close']
        rate = irx.iloc[0] / 100  # IRX в процентах
        return float(rate)
    except Exception:
        return None
...
def get_close_prices_div_yield_sigma_rf_strikes(tickers, date1, date2):
    ...
    price_data = yf.download(tickers, start=full_start, end=full_end)['Close']
